...
export const userjobdata = async (req, res, next) => {
    try {
        const data = await Job.find({JobStatus: true});
...
export const Resumefile = async (req, res,  next) => {
    if (req.params.id === req.user.id) { 
        ...
        try {
            ...
            return res.status(200).send({ message: "Successfully Resume Updated" });
...
export const uploadImage= async(req,res,next)=>{
    ...
    try{
        ...
        return res.status(200).send({message:"uploaded",blogid,imagepath}).sendFile(path+imagepath,imagename);
    }catch(error){
