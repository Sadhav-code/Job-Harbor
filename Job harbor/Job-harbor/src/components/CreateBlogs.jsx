...
export default function CreaeBlogs() {
    ...
    const handleUpload = () => {
        if (file == null) return;
        const imageref = ref(storage, `blogimage/${file.name + v4()}`);
        uploadBytes(imageref, file).then(() => {
            alert("image uploaded")
            getDownloadURL(imageref).then((url) => {
                setphotoid(url)
            }).catch(error => {
                console.log(error, message, "error getting in image url ")
            })
        })
    }
    ...
    return (
        <div className="card mb-4">
            ...
            <div className="card-body">
                <a href="#!"><img className="card-img-top" src={photoid} height="100px" width="100px" /></a>
